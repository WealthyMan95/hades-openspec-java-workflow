...
Java 编码规范。写代码时先用它，Review 时再结合 Hades 的 Review Gate 和后续 Java 后端评审规则。

## 优先级

1. repo 本地规则：formatter、Checkstyle、Spotless、PMD、Sonar、现有风格。
2. `AGENTS.md`、PRD、OpenSpec、`tasks.md`。
3. Google Java Style：格式、import、括号、命名。
4. 阿里 Java 开发手册：异常、日志、集合、并发、SQL、安全、测试。
5. 涉及 Mapper/SQL/查询接口时，先按 SQL 性能规则设计和实现。
...
- 设计、编写或修改 Mapper/SQL、列表、搜索、统计、导出、分页、排序时，必须先遵循 SQL 性能规则，不是等到 Review 再补。
- 用户输入必须参数绑定。
